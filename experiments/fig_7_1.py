...
n_samples = 16
pd = 0
pl_list = list(np.linspace(0.99, 0.995, math.floor(n_samples/4))) + \
          list(np.linspace(0.996, 0.9985, math.floor(n_samples/4))) +\
          list(np.linspace(0.9986, 0.9994, math.floor(n_samples/4))) +\
          list(np.linspace(0.9995, 1.0, math.ceil(n_samples/4)))
...
for i in range(n_samples):
    ...
    densities = run_simulation(grid, pd, pl_list[i], i, n_samples)
    last_1000 = densities[-1000:]
    rounded_last_1000 = np.round(last_1000, 3)
    mode_result = stats.mode(rounded_last_1000)
    mode_value = mode_result.mode[0]
    print(f"\nMode density (pl = {round(pl_list[i], 4)}): {mode_value}")
    phi_list.append(mode_value)


plt.plot(pl_list, phi_list)
plt.title('Life density \u03A6 along edge (p$_d$ = 0, p$_l$)')
plt.xlabel('p$_l$')
plt.ylabel('\u03A6')
plt.ylim(0, 0.1)
plt.show()
